...
# ╔═╡ 7d245f4c-a693-11ef-22d4-0bac849dffd8
begin
	using Euler2D
	using ForwardDiff
	using LaTeXStrings
...
# ╔═╡ 84f144b2-bcda-41ea-8014-a31674890696
tangent=load_cell_sim("../data/circular_obstacle_tangent.celltape");

# ╔═╡ 33077a7b-def8-49b7-a4d5-e1ed33e9f8ed
begin
	bcurve(y) = ShockwaveProperties.BilligShockParametrization.shock_front(y, 4.0, 0.75)
	billig_data = mapreduce(vcat, bcurve.(cell_centers(tangent)[2])) do v
		v'
	end
end

# ╔═╡ 6f31bd03-7778-46f3-8838-7376cff88bc0
@bind t Slider((0:0.001:1.)[1:end-1], show_value=true)

# ╔═╡ b15c2e22-d4b8-4552-b59a-0501ac4ec77d
@bind n Slider(1:n_tsteps(tangent), show_value=true)

# ╔═╡ 0aff1856-0442-4fd7-a6d7-8da6a33afecb
pfield = map(pressure_field(tangent, n, DRY_AIR)) do val
	isnothing(val) ? missing : val
end

# ╔═╡ a9809201-da17-4a89-a30b-f50985eb7b16
heatmap(pfield', aspect_ratio=:equal, xlims=(1,400), ylims=(1,600))

# ╔═╡ 6fb89cad-7f0d-40fb-a28c-7a922a9ea7e2
function dPdp(sim::CellBasedEulerSim{T, C}, n, pidx) where {T, C<:Euler2D.TangentQuadCell}
	_, u_cells = nth_step(sim, n)
	res = Array{Union{T, Missing}}(missing, grid_size(sim))
	for i∈eachindex(sim.cell_ids)
		sim.cell_ids[i] == 0 && continue
		cell = u_cells[sim.cell_ids[i]]
		dP = ForwardDiff.gradient(cell.u) do u
			Euler2D._pressure(u, DRY_AIR)
		end
		res[i] = (dP' * cell.u̇)[pidx]
	end
	return res 
end

# ╔═╡ d2ab0d4c-bc67-44d9-af96-06e118ca970d
dPdρinf = dPdp(tangent, n, 1);

# ╔═╡ a9783247-3768-492c-852a-4a8e19bbfc1f
heatmap(dPdρinf', aspect_ratio=:equal, xlims=(1,400), ylims=(1,600), dpi=1000, size=(500,500))

# ╔═╡ 7f301de2-b55b-487a-8253-fe68a5cd05bc
function dMdp(sim::CellBasedEulerSim{T, C}, n, pidx) where {T, C<:Euler2D.TangentQuadCell}
	_, u_cells = nth_step(sim, n)
	res = Array{Union{T, Missing}}(missing, grid_size(sim))
	for i∈eachindex(sim.cell_ids)
		sim.cell_ids[i] == 0 && continue
		cell = u_cells[sim.cell_ids[i]]
		dM = ForwardDiff.gradient(cell.u) do u
			a = Euler2D.dimensionless_speed_of_sound(u, DRY_AIR)
			M = norm(Euler2D.select_middle(cell.u)/(cell.u[1]*a))
			return M
		end
		res[i] = (dM' * cell.u̇)[pidx]
	end
	return res 
end
...
manifest_format = "2.0"
project_hash = "ea7ffc59548210ba975563438360137bf4173d6c"
